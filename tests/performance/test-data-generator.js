...
class TestDataGenerator {
    constructor() {
        this.categories = ['Income', 'Food', 'Transportation', 'Housing', 'Utilities', 'Entertainment', 'Healthcare', 'Debt', 'Savings', 'Shopping'];
        this.merchants = ['Amazon', 'Walmart', 'Target', 'Starbucks', 'McDonalds', 'Shell', 'Costco', 'Home Depot', 'Best Buy', 'Kroger'];
    ...
    generateSavingsGoals(count) {
        ...
        for (let i = 1; i <= count; i++) {
            ...
            goals.push({
                ...
                completed_date: currentAmount >= targetAmount ? new Date().toISOString().split('T')[0] : null,
                completedDate: currentAmount >= targetAmount ? new Date().toISOString().split('T')[0] : null
            });
        }
        return goals;
    }

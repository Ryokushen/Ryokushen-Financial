...
class BenchmarkReportGenerator {
  ...
  generateReport(options = {}) {
    const { format = 'html', includeRawData = false, compareWithBaseline = true } = options;
  ...
  generateSummary() {
    ...
    const passedTests = Array.from(this.testResults.values()).filter(
      result => result.validation?.passed || result.success
    ).length;
...
|--------|---------|--------|--------|
${Object.entries(report.summary.averageMetrics)
...
|------------|--------|----------|--------|
${Object.entries(report.detailedResults)
  .map(
    ([suite, result]) =>
...
`;
  ...
  analyzeDataIndexPerformance() {
    const dataIndexResults = Array.from(this.testResults.values()).filter(
      result => result.testType === 'dataIndex' || result.metrics?.dataIndexRebuild
    );
  ...
  analyzeUIPerformance() {
    const uiResults = Array.from(this.testResults.values()).filter(
      result => result.testType === 'ui' || result.metrics?.chartRender
    );
  ...
  analyzeMemoryPerformance() {
    const memoryResults = Array.from(this.testResults.values()).filter(
  ...
  calculateTotalTestDuration() {
    return Array.from(this.testResults.values()).reduce(
  ...
  calculateMemoryUsage() {
    ...
    if (memoryResults.length === 0) {
      return { baseline: 0, peak: 0, average: 0 };
    }
  ...
  calculateBrowserCompatibilityScore() {
    const compatibilityResults = Array.from(this.testResults.values()).filter(
    ...
    const passedTests = compatibilityResults.filter(
      result => result.validation?.passed || result.success
    ).length;

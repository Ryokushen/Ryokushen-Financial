...
async function runTransactionManagerTests() {
  ...
  try {
    ...
    if (isLoginPage) {
      ...
      console.log(
        tmLoaded ? '✅ TransactionManager loaded in app' : '❌ TransactionManager not loaded'
      );
      ...
      console.log(
        usesNewSystem
          ? '✅ Transactions module uses TransactionManager'
          : '❌ Transactions module not updated'
      );
      ...
      const consoleTest = await page.evaluate(async () => {
        try {
          if (!window.transactionManager) {
            return { success: false, error: 'TransactionManager not found' };
          }

...
(async function initApp() {
  ...
  async function initializeApp() {
    ...
    window.searchTransactions = async filters => {
      try {
        ...
        console.log(
          `Found ${results.metadata.totalCount} transactions (showing ${results.metadata.returnedCount})`
        );
        console.log(
          `Total: $${results.metadata.totalAmount.toFixed(2)} (Income: $${results.metadata.totalIncome.toFixed(2)}, Expense: $${results.metadata.totalExpense.toFixed(2)})`
        );
        console.table(
  ...
  async function loadAllData() {
    try {
      ...
      const investmentAccounts =
        investmentAccountsResult.status === 'fulfilled' ? investmentAccountsResult.value : [];
      const cashAccounts =
        cashAccountsResult.status === 'fulfilled' ? cashAccountsResult.value : [];
      const debtAccounts =
        debtAccountsResult.status === 'fulfilled' ? debtAccountsResult.value : [];
      const recurringBills =
        recurringBillsResult.status === 'fulfilled' ? recurringBillsResult.value : [];
      const savingsGoals =
        savingsGoalsResult.status === 'fulfilled' ? savingsGoalsResult.value : [];
      ...
      appState.appData.investmentAccounts = investmentAccounts.map(
        Investments.mapInvestmentAccount
      );
      ...
      appState.appData.recurringBills = recurringBills.map(b => ({
        ...
        is_active:
          b.is_active !== undefined ? b.is_active : b.active !== undefined ? b.active : true,
    ...
    privacyManager.addListener(() => {
      ...
      setTimeout(() => {
        ...
        if (activeTabId === 'dashboard') {
          ...
          window.lastInvestmentData &&
          window.lastInvestmentChartType
        ) {
  ...
  async function updateAllDisplays(state, options = {}) {
    ...
    if (forceAll || !activeTab) {
      ...
      if (debtList) {
        await Debt.renderDebtAccounts(state);
      }
  ...
  async function migrateDebtTransactions(transactions, debtAccounts) {
    ...
    for (const t of transactions.filter(
      t => t.category === 'Debt' && t.debt_account && !t.debt_account_id
    )) {
  ...
  window.cleanupApp = () => {
    ...
    if (typeof Accounts.cleanup === 'function') {
      Accounts.cleanup();
    }
    if (typeof Transactions.cleanup === 'function') {
      Transactions.cleanup();
    }
    if (typeof Investments.cleanup === 'function') {
      Investments.cleanup();
    }
    if (typeof Debt.cleanup === 'function') {
      Debt.cleanup();
    }
    if (typeof Recurring.cleanup === 'function') {
      Recurring.cleanup();
    }
    if (typeof Savings.cleanup === 'function') {
      Savings.cleanup();
    }

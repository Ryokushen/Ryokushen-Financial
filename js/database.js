...
class FinancialDatabase {
  ...
  async addCashAccount(account) {
    try {
      ...
      this.handleError('addCashAccount', error);
    }
  }
  ...
  async updateCashAccount(id, account) {
    try {
      ...
      this.handleError('updateCashAccount', error);
    }
  }
  ...
  async deleteCashAccount(id) {
    ...
    try {
      ...
      if (accountError) {
        ...
        if (
          recurringBillResult.status === 'fulfilled' &&
          recurringBillsToUpdate &&
          recurringBillsToUpdate.length > 0
        ) {
          const billRestorePromises = recurringBillsToUpdate.map(bill =>
            this.supabase.from('recurring_bills').update({ account_id: id }).eq('id', bill.id)
          );
        ...
        if (restorePromises.length > 0) {
          ...
          throw new Error(
            `Account deletion failed: ${accountError.message}. Related data has been restored where possible.`
          );
        } else {
  ...
  async addTransaction(transaction) {
    try {
      ...
      this.handleError('addTransaction', error);
    }
  }
  ...
  async updateTransaction(id, updates) {
    try {
      ...
      this.handleError('updateTransaction', error);
    }
  }
  ...
  async addTransactionTemplate(template) {
    try {
      ...
      this.handleError('addTransactionTemplate', error);
    }
  }
  ...
  async updateTransactionTemplate(id, updates) {
    try {
      ...
      this.handleError('updateTransactionTemplate', error);
    }
  }
  ...
  async deleteTransactionTemplate(id) {
    try {
      ...
      this.handleError('deleteTransactionTemplate', error);
    }
  }
  ...
  async addInvestmentAccount(account) {
    try {
      ...
      this.handleError('addInvestmentAccount', error);
    }
  }
  ...
  async updateInvestmentAccount(id, account) {
    try {
      ...
      this.handleError('updateInvestmentAccount', error);
    }
  }
      ...
      if (accountError) {
        ...
        if (holdingsToDelete && holdingsToDelete.length > 0) {
          try {
            ...
            throw new Error(
              `Account deletion failed: ${accountError.message}. Holdings have been restored.`
            );
          } catch (restoreError) {
            throw new Error(
              'Account deletion failed AND holdings restoration failed. Data may be in inconsistent state.'
            );
          }
        }
        throw new Error(`Account deletion failed: ${accountError.message}`);
  ...
  async addHolding(accountId, holding) {
    try {
      ...
      this.handleError('addHolding', error);
    }
  }
  ...
  async updateHolding(id, holding) {
    try {
      ...
      this.handleError('updateHolding', error);
    }
  }
  ...
  async deleteHolding(id) {
    try {
      ...
      this.handleError('deleteHolding', error);
    }
  }
  ...
  async addDebtAccount(debtData) {
    try {
      ...
      this.handleError('addDebtAccount', error);
    }
  }
  ...
  async updateDebtAccount(id, debtData) {
    try {
      ...
      this.handleError('updateDebtAccount', error);
    }
  }
  ...
  async deleteDebtAccount(id) {
    try {
      ...
      this.handleError('deleteDebtAccount', error);
    }
  }
  ...
  async updateDebtBalance(debtId, newBalance) {
    try {
      ...
      this.handleError('updateDebtBalance', error);
    }
  }
  ...
  async getCashAccountById(id) {
    try {
      ...
      this.handleError('getCashAccountById', error);
    }
  }
  ...
  async getDebtAccountById(id) {
    try {
      ...
      this.handleError('getDebtAccountById', error);
    }
  }
  ...
  async deleteRecurringBill(id) {
    try {
      ...
      this.handleError('deleteRecurringBill', error);
    }
  }
  ...
  async addSavingsGoal(goalData) {
    try {
      ...
      this.handleError('addSavingsGoal', error);
    }
  }
  ...
  async updateSavingsGoal(id, goalData) {
    try {
      ...
      this.handleError('updateSavingsGoal', error);
    }
  }
  ...
  async deleteSavingsGoal(id) {
    try {
      ...
      this.handleError('deleteSavingsGoal', error);
    }
  }
  ...
  async getSmartRules(enabled = null) {
    try {
      ...
      this.handleError('getSmartRules', error);
    }
  }
  ...
  async createSmartRule(ruleData) {
    try {
      ...
      this.handleError('createSmartRule', error);
    }
  }
  ...
  async updateSmartRule(id, updates) {
    try {
      ...
      this.handleError('updateSmartRule', error);
    }
  }
  ...
  async deleteSmartRule(id) {
    try {
      ...
      this.handleError('deleteSmartRule', error);
    }
  }
  ...
  async createPaySchedule(scheduleData) {
    try {
      ...
      this.handleError('createPaySchedule', error);
    }
  }
  ...
  async updatePaySchedule(id, scheduleData) {
    try {
      ...
      this.handleError('updatePaySchedule', error);
    }
  }

  async deletePaySchedule(id) {
    try {
      const { error } = await this.supabase.from('pay_schedules').delete().eq('id', id);
      if (error) {
        throw error;
      }
      return true;
    } catch (error) {
      this.handleError('deletePaySchedule', error);
    }
  }
  ...
  async captureFinancialSnapshot(snapshotType = 'daily') {
    try {
      const userId = await this.getCurrentUserId();
      const { data, error } = await this.supabase.rpc('capture_financial_snapshot', {
        p_user_id: userId,
  ...
  async getSnapshotComparison(comparisonType = 'month') {
    try {
      const userId = await this.getCurrentUserId();
      const { data, error } = await this.supabase.rpc('get_snapshot_comparison', {
        p_user_id: userId,
  ...
  async getDebtHistoricalChange(debtAccountId = null) {
    try {
      ...
      if (
        debtAccountId &&
        current.account_details?.debt_accounts &&
        previous.account_details?.debt_accounts
      ) {
        // Calculate change for specific debt account
        const currentAccount = current.account_details.debt_accounts.find(
          a => a.id === debtAccountId
        );
        const previousAccount = previous.account_details.debt_accounts.find(
          a => a.id === debtAccountId
        );
  ...
  async processRecurringPayment(billId) {
    return this.executeWithRetry(async () => {
      const userId = await this.getCurrentUserId();
      const { data, error } = await this.supabase.rpc('process_recurring_payment', {
        p_bill_id: billId,
        p_user_id: userId,
      });
  ...
  async bulkImportTransactions(transactions) {
    return this.executeWithRetry(async () => {
      const userId = await this.getCurrentUserId();
      const { data, error } = await this.supabase.rpc('bulk_import_transactions', {
        p_transactions: transactions, // Pass array directly, Supabase will handle JSONB conversion
        p_user_id: userId,
      });

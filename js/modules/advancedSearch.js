...
class AdvancedSearch {
  ...
  clearSearch() {
    ...
    if (this.fuzzyCheckbox) {
      this.fuzzyCheckbox.checked = true;
    }
    if (this.dateFrom) {
      this.dateFrom.value = '';
    }
  ...
  buildFilters() {
    ...
    if (this.amountMin.value) {
      filters.amountMin = parseFloat(this.amountMin.value);
    }
    if (this.amountMax.value) {
      filters.amountMax = parseFloat(this.amountMax.value);
    }
    if (this.categorySelect.value) {
      filters.category = this.categorySelect.value;
    }
  ...
  displayResults() {
    ...
    // Render results
    this.resultsContainer.innerHTML = pageResults
      .map(
        transaction => `
  ...
  renderSavedSearches() {
    ...
    if (this.savedSearches.length === 0) {
      this.savedSearchesList.innerHTML =
        '<div class="empty-state"><p>No saved searches yet</p></div>';
      return;
    }

    this.savedSearchesList.innerHTML = this.savedSearches
      .map(
        search => `
  ...
  getSearchSummary(criteria) {
    ...
    if (criteria.filters?.dateFrom || criteria.filters?.dateTo) {
  ...
  async loadSavedSearch(searchId) {
    ...
    if (this.searchText) {
      this.searchText.value = search.criteria.text || '';
    }
    if (this.fuzzyCheckbox) {
      this.fuzzyCheckbox.checked = search.criteria.fuzzy || false;
    }
    if (this.queryTextarea) {
      this.queryTextarea.value = search.criteria.query || '';
    }
    ...
    if (this.amountMin) {
      this.amountMin.value = filters.amountMin || '';
    }
    if (this.amountMax) {
      this.amountMax.value = filters.amountMax || '';
    }
    if (this.categorySelect) {
      this.categorySelect.value = filters.category || '';
    }
    if (this.accountSelect) {
      this.accountSelect.value = filters.accountId || '';
    }

...
function createDebtHealthGauge({ appData, CHART_COLORS }) {
  ...
  if (dti <= 20) {
    needleColor = CHART_COLORS[0];
  } // Excellent (teal)
  else if (dti <= 35) {
    needleColor = CHART_COLORS[7];
  } // Good (brown/orange)
  else if (dti <= 43) {
    needleColor = CHART_COLORS[6];
  } // Fair (yellow)
  ...
  try {
    ...
    if (activeTabId === 'dashboard') {
      ...
      updateOrCreateChart(
        'debtHealthGauge',
        () =>
          createDebtHealthGauge({ appData: appState.appData, CHART_COLORS: appState.CHART_COLORS }),
        appState
      );
      ...
      updateOrCreateChart(
        ...
        () =>
          createExpenseCategoryChart({
            ...
            CHART_COLORS: appState.CHART_COLORS,
          }),
        appState
      );
      updateOrCreateChart(
        'cashFlowChart',
        () =>
          createCashFlowChart({ appData: appState.appData, CHART_COLORS: appState.CHART_COLORS }),
        appState
      );
      updateOrCreateChart(
        'assetsDebtChart',
        () =>
          createAssetsDebtChart({ appData: appState.appData, CHART_COLORS: appState.CHART_COLORS }),
        appState
      );
...
function updateChartData(chartName, appState) {
  ...
  switch (chartName) {
    case 'debtHealthGauge':
      ...
      if (dti <= 20) {
        needleColor = appState.CHART_COLORS[0];
      } else if (dti <= 35) {
        needleColor = appState.CHART_COLORS[7];
      } else if (dti <= 43) {
        needleColor = appState.CHART_COLORS[6];
      }
      ...
      appData.transactions
        .filter(
          t =>
            ...
            (t.amount > 0 && t.debt_account_id && t.category !== 'Payment')
        )
        .forEach(t => {
    ...
    case 'assetsDebtChart':
      const totalCash = appData.cashAccounts.reduce(
        (sum, account) => sum + (account.balance || 0),
        0
      );
      const totalInvestments = appData.investmentAccounts.reduce(
...
function createCashFlowChart({ appData, CHART_COLORS }) {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    expenseData.push(
      Math.abs(
        monthTransactions
          .filter(t => t.amount < 0 || (t.amount > 0 && t.debt_account_id))
...
function createPayoffTimelineChart({ appData, CHART_COLORS }) {
  ...
  const timeline = DebtStrategy.calculatePayoffTimeline(
    appData.debtAccounts,
    strategy,
    extraPayment
  );
      ...
      datasets: [
        {
          ...
          data: scenarios.map(s =>
            chartType === 'contribution' ? s.futureValue : s.requiredMonthlyContribution
          ),
          backgroundColor: CHART_COLORS[0],
          ...
          tooltip: {
            callbacks: {
              label(context) {
                ...
                const originalValue =
                  context.dataIndex === 0
                    ? middleScenario.totalContributions
                    : middleScenario.projectedEarnings;

                if (originalValue === 0) {
                  return isPrivacyMode()
                    ? `${label}: $*** (None needed!)`
                    : `${label}: $0.00 (None needed!)`;
                }

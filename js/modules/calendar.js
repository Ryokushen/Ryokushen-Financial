...
export class Calendar {
  ...
  getMonthSummary() {
    const monthEvents = this.events.filter(event => {
      return event.date.getMonth() === this.currentMonth &&
             event.date.getFullYear() === this.currentYear
    })

    const totalBills = monthEvents.filter(e => e.type === 'bill').reduce((sum, e) => sum + e.amount, 0)
    const totalPay = monthEvents.filter(e => e.type === 'pay').reduce((sum, e) => sum + e.amount, 0)

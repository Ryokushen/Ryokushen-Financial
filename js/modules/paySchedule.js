...
class PaySchedule {
  ...
  async updateSchedule(id, scheduleData) {
    try {
      ...
      window.dispatchEvent(
        new CustomEvent('paySchedule:updated', {
    ...
    switch (schedule.frequency) {
      case 'weekly':
        return this.getNextWeeklyDate(
          startDate,
          schedule.day_of_week || startDate.getDay(),
          fromDate
        );
  ...
  getNextSemiMonthlyDate(day1, day2, fromDate) {
    ...
    return new Date(
      nextYear,
      nextMonth,
      Math.min(days[0], this.getDaysInMonth(nextYear, nextMonth))
    );
  }
  ...
  getNextMonthlyDate(dayOfMonth, fromDate) {
    ...
    if (dayOfMonth > currentDay) {
      ...
      return new Date(
        nextYear,
        nextMonth,
        Math.min(dayOfMonth, this.getDaysInMonth(nextYear, nextMonth))
      );
    }
  }

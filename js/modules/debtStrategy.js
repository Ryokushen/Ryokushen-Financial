...
export const DebtStrategy = (() => {
  ...
  const calculatePayoffTimeline = (debts, strategy = 'avalanche', extraPayment = 0) => {
    ...
    const sortedDebts =
      strategy === 'snowball' ? calculateSnowballOrder(debts) : calculateAvalancheOrder(debts);
    ...
    const totalMinimumPayment = workingDebts.reduce(
    ...
    while (workingDebts.some(debt => debt.remainingBalance > 0)) {
      ...
      for (const debt of workingDebts) {
        ...
        const monthlyInterest = calculateMonthlyInterest(debt.remainingBalance, debt.interestRate);
        ...
        if (availableExtra > 0 && !debt.paidOff) {
          const extraForThisDebt = Math.min(
            availableExtra,
            debt.remainingBalance + monthlyInterest - payment
          );
        ...
        if (debt.remainingBalance <= 0.01) {
          // Using 0.01 to handle floating point
  ...
  const calculateCreditUtilization = debts => {
    const creditCards = debts.filter(debt => debt.type === 'Credit Card' && debt.creditLimit > 0);

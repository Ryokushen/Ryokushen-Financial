...
function populateTransferToAccount(type, excludeAccount = null) {
  ...
  if (!appStateReference || !appStateReference.appData) {
    return;
  }
  ...
  if (type === 'payment') {
    ...
    const activeAccounts = cashAccounts.filter(
      a => a.isActive && `cash_${a.id}` !== excludeAccount
    );
  ...
  if (transactionAccount) {
    eventManager.addEventListener(transactionAccount, 'change', function () {
      ...
      if (categoryValue === 'Debt' && this.value && this.value.startsWith('cc_')) {
        showError(
          "For credit card transactions, please use a regular category instead of 'Debt'. The 'Debt' category is for non-account debt payments."
        );
      ...
      if (categoryValue === 'Payment' && this.value && this.value.startsWith('cc_')) {
        showError(
          'For payments, please select a cash account as the source. Credit card accounts cannot be used to pay other debts.'
        );
    ...
    eventManager.addEventListener(transactionCategory, 'change', function () {
      ...
      if (this.value === 'Debt') {
        ...
        if (this.value === 'Payment') {
          if (accountLabel) {
            accountLabel.textContent = 'From Account (Cash)';
          }
      ...
      if (this.value === 'Debt' && accountValue && accountValue.startsWith('cc_')) {
        showError(
          "For credit card transactions, please use a regular category instead of 'Debt'. For payments, use the 'Payment' category. The 'Debt' category is for non-account debt payments."
        );
...
async function setupVoiceInput() {
  ...
  if (!voiceButton || !descriptionInput) {
    return;
  }
          ...
          onSmartParsed: smartResult => {
            ...
            if (fillResult.success) {
              announceToScreenReader(
                `Smart voice input filled ${fillResult.fieldsChanged.length} fields`
              );
            }
          },
          onError: error => {
...
function editTransaction(id, appState) {
  ...
  if (transaction.debt_account_id && !transaction.account_id) {
    ...
    if (transaction.amount < 0) {
      ...
      debug.log(
        'Edit mode - Credit card purchase:',
        transaction.amount,
        'displayed as:',
        formData.amount
      );
  ...
  if (transaction.category === 'Debt' && transaction.debt_account_id) {
    const debtAccount = appState.appData.debtAccounts.find(
      d => d.id === transaction.debt_account_id
    );
    if (debtAccount) {
  ...
  if (accountField) {
    accountField.value = formData.account;
  }
  if (categoryField) {
    categoryField.value = formData.category;
  }
  if (descriptionField) {
    descriptionField.value = formData.description;
  }
  if (amountField) {
    amountField.value = formData.amount;
  }
  if (clearedField) {
    clearedField.checked = formData.cleared;
  }
...
async function handleTransactionSubmit(event, appState, onUpdate) {
  ...
  try {
    ...
    if (
      isCredit &&
      transactionData.category !== 'Payment' &&
      ...
      if (
        ...
        !confirm('Negative amount will decrease the debt balance (e.g., a payment). Proceed?')
      ) {
        return;
      } else if (
        transactionData.amount > 0 &&
        !confirm('Positive amount will increase the debt balance (e.g., a charge). Proceed?')
      ) {
        return;
      }
    } else {
    ...
    if (
      (transactionData.category === 'Payment' || transactionData.category === 'Transfer') &&
      ...
      if (!transferToValue) {
        showError(
          `Please select a destination account for the ${transactionData.category.toLowerCase()}.`
        );
        return;
      }
...
async function addLinkedTransaction(fromTransactionData, toAccountValue, appState, onUpdate) {
  try {
    ...
    const fromAccountName = getAccountName(
      fromTransactionData.account_id,
      fromTransactionData.debt_account_id,
      appState
    );
...
async function addNewTransaction(transactionData, appState, onUpdate) {
  try {
    ...
    if (newTransaction.debt_account_id) {
      ...
      if (updatedAccount) {
        const debtAccount = appState.appData.debtAccounts.find(
          d => d.id === newTransaction.debt_account_id
        );
        if (debtAccount) {
...
export function renderTransactions(appState, categoryFilter = currentCategoryFilter) {
  ...
  if (transactions.length === 0) {
    tbody.innerHTML =
      '<div class="empty-state" style="grid-column: 1 / -1; text-align: center; padding: 3rem;">No transactions found</div>';
    return;
  }
...
async function deleteTransaction(id, appState, onUpdate) {
  ...
  if (
    confirm(
      'Are you sure you want to delete this transaction? This will adjust account balances and potentially revert recurring bill payments.'
    )
  ) {
    try {
      ...
      if (transactionToDelete.debt_account_id) {
        ...
        if (updatedAccount) {
          const debtAccount = appState.appData.debtAccounts.find(
            d => d.id === transactionToDelete.debt_account_id
          );
          if (debtAccount) {
            debtAccount.balance = updatedAccount.balance;
          }
...
async function handleRecurringBillReversion(deletedTransaction, appState) {
  ...
  const billName = deletedTransaction.description
    .replace(' (Recurring)', '')
    .replace(' (Recurring - Credit Card)', '');

  const recurringBill = appState.appData.recurringBills.find(
    bill => bill.name === billName || deletedTransaction.description.startsWith(bill.name)
  );

  if (!recurringBill) {
    debug.warn(
      'Could not find matching recurring bill for transaction:',
      deletedTransaction.description
    );
    return;
  }

...
class FinancialCalculations {
  ...
  static calculateLoanPayoff(balance, rate, payment, extraPayment = 0) {
    ...
    return {
      ...
      payoffDate,
      monthlySavings:
        extraPayment > 0 ? this.calculateMonthlySavings(balance, rate, payment, extraPayment) : 0,
    };
  }
  ...
  static projectBalance(currentBalance, transactions, recurringBills, daysToProject) {
    ...
    const recentTransactions = transactions.filter(t => new Date(t.date) >= recentDate);
  ...
  static analyzeCategoryTrend(transactions, category, months = 6) {
    ...
    const percentChange = firstMonth === 0 ? 0 : ((lastMonth - firstMonth) / firstMonth) * 100;

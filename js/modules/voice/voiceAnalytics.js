...
export class VoiceAnalytics {
  ...
  async handleSavingsRateQuery(parameters) {
    ...
    try {
      ...
      let interpretation = '';
      if (savingsRate >= 20) {
        interpretation = "Excellent! You're saving well.";
      } else if (savingsRate >= 10) {
        interpretation = 'Good savings rate.';
      } else if (savingsRate >= 5) {
        interpretation = 'Room for improvement.';
      } else {
        interpretation = 'Consider increasing your savings.';
      }
  ...
  async handleExpenseIncreaseQuery(parameters) {
    ...
    return {
      ...
      response: {
        ...
        details: topIncreases
          .map(cat => `${cat.category}: +${formatCurrency(cat.increase)}`)
          .join(' | '),
      },
    };
  }
  ...
  async handleTimeSpendingQuery(parameters) {
    ...
    const periodTransactions = appData.transactions.filter(
      t => this.isTransactionInTimeRange(t, timeRange) && t.amount < 0
    );
  ...
  async handleOverdueBillsQuery(parameters) {
    ...
    return {
      ...
      response: {
        ...
        details:
          overdueBills.length > 0
            ? overdueBills.map(bill => `${bill.name}: ${formatCurrency(bill.amount)}`).join(' | ')
  ...
  calculatePeriodSpending(transactions, timeRange) {
    const expenses = transactions.filter(
      t => this.isTransactionInTimeRange(t, timeRange) && t.amount < 0 && t.category !== 'Transfer'
    );
  ...
  async handleHighestInterestQuery(parameters) {
    ...
    const debtsWithInterest = appData.debtAccounts.filter(
      d => d.interestRate && d.interestRate > 0 && d.balance > 0
    );
  ...
  async handleCreditUtilizationQuery(parameters) {
    ...
    const creditCards = appData.debtAccounts.filter(d => d.creditLimit && d.creditLimit > 0);
  ...
  async handleGoalProgressQuery(parameters) {
    ...
    const goalProgress = activeGoals.map(goal => {
      ...
      const isOnTrack = goal.targetDate ? this.calculateIfOnTrack(goal) : true;
  ...
  async handleInvestmentPerformanceQuery(parameters) {
      ...
      if (account.holdings) {
        account.holdings.forEach(holding => {
          allHoldings.push({
            ...
            percentChange:
              holding.dayChange && holding.value
                ? (holding.dayChange / (holding.value - holding.dayChange)) * 100
                : 0,
          });
        });
      }
    ...
    const sortedByPerformance = [...allHoldings].sort(
      (a, b) => (b.dayChange || 0) - (a.dayChange || 0)
    );
  ...
  async handleFindTransactionsQuery(parameters) {
    ...
    return {
      ...
      response: {
        ...
        details:
          count > 5 ? `Showing latest 5 of ${count} transactions` : `All ${count} transactions`,
      },
    };
  }
  ...
  async handleRecurringChargesQuery(parameters) {
    ...
    const allRecurring = [
      ...
      ...potentialRecurring
        .filter(p => !activeBills.some(b => b.name.toLowerCase() === p.merchant.toLowerCase()))
        .map(p => ({
          name: p.merchant,
    ...
    const totalMonthly = sumMoney(
      allRecurring.map(r => this.convertToMonthly(r.amount, r.frequency))
    );
  ...
  detectRecurringCharges(transactions) {
    ...
    for (const [merchant, charges] of Object.entries(merchantPatterns)) {
      if (charges.length >= 3) {
        ...
        const variance =
          amounts.reduce((sum, amt) => sum + Math.abs(amt - avgAmount), 0) / amounts.length;
  ...
  getSupportedQueries() {
    return {
      'Balance Queries': ["What's my balance?", 'Show total cash', 'How much money do I have?'],
      'Net Worth Queries': ["What's my net worth?", 'Show my total wealth', 'How much am I worth?'],
      'Debt Queries': ['How much debt do I have?', 'Show my debt', 'What do I owe?'],
      'Investment Queries': [
  ...
  async handleGoalTimelineQuery(parameters) {
    try {
      ...
      const recentContributions = transactions.filter(
        t => new Date(t.date) >= recentDate && t.category === 'Savings' && t.amount > 0
      );
      ...
      if (monthlyAverage <= 0) {
        return {
          ...
          response: {
            ...
            title: 'Goal Timeline',
            details:
              "Based on your recent saving history, you haven't been contributing to this goal.",
          },
        };
      }
  ...
  async handleHighestCategoryQuery(parameters) {
    try {
      ...
      const periodTransactions = transactions.filter(t => {
        ...
        return tDate >= dateRange.start && tDate <= dateRange.end && t.amount < 0; // Only expenses
      });
  ...
  async handleComparativeSpendingQuery(parameters) {
    try {
      ...
      const baseTransactions = transactions.filter(t => {
        const tDate = new Date(t.date);
        return tDate >= baseRange.start && tDate <= baseRange.end && t.amount < 0;
      });

      const compareTransactions = transactions.filter(t => {
        const tDate = new Date(t.date);
        return tDate >= compareRange.start && tDate <= compareRange.end && t.amount < 0;
      });
      ...
      const percentChange =
        compareTotal === 0 ? 100 : Math.round(((baseTotal - compareTotal) / compareTotal) * 100);
  ...
  async handleAnomalyDetectionQuery(parameters) {
    try {
      ...
      const recentTransactions = transactions.filter(
        t => new Date(t.date) >= recentDate && t.amount < 0
      );
      ...
      return {
        ...
        response: {
          ...
          details: topAnomalies
            .slice(1, 3)
            .map(
              a => `${a.transaction.description}: ${formatCurrency(Math.abs(a.transaction.amount))}`
            )
            .join(', '),
          emoji: '⚠️',
        },
  ...
  async handleWorkTimeCostQuery(parameters) {
    try {
      ...
      if (workTimeCategory) {
        const categoryTransactions = (appData.transactions || []).filter(
          t =>
            t.amount < 0 &&
            t.category &&
      ...
      const thisMonthExpenses = (appData.transactions || []).filter(t => {
        const transDate = new Date(t.date);
        const now = new Date();
        return (
          t.amount < 0 &&

...
class PerformanceDashboard {
    ...
    renderMetricCards() {
        ...
        const cards = [
            ...
            },
            {
                ...
                trend: predictions.predictions[0]?.total > summary.averageMonthlySpending ? 'negative' : 'positive',
    ...
    calculateTrendPercentage(summary) {
        ...
        if (!summary || !summary.trendAnalysis) return '0%';
        
        const { percentageChange } = summary.trendAnalysis || {};
        if (percentageChange === undefined) return '0%';
    ...
    showLoadingState() {
        ...
        if (metricGrid) {
            metricGrid.innerHTML = Array(6).fill('').map(() => `
                <div class="metric-card">
    ...
    renderAnomalyAlerts() {
        ...
        if (!container || !this.data.anomalies) return;
        ...
        container.innerHTML = topAnomalies.map(anomaly => `
    ...
    renderDataQuality() {
        ...
        if (!container || !this.data.dataQuality) return;
    ...
    renderSystemPerformance() {
        ...
        if (!container || !this.data.systemMetrics) return;

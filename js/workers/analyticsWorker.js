...
function analyzeTransactions({ transactions, options = {} }) {
  ...
  for (const [merchant, txns] of Object.entries(byMerchant)) {
    if (txns.length >= 3) { // Only include frequent merchants
      merchantStats.push({
        merchant,
...
function predictSpending({ transactions, daysAhead = 30 }) {
  ...
  const byCategory = groupBy(transactions.filter(t => t.amount < 0), 'category');
  ...
  const totalDailySpending = Object.values(categoryPatterns)
    .reduce((sum, pattern) => sum + pattern.dailyAverage, 0);
...
function analyzeCategories({ transactions, options = {} }) {
  ...
  const totalExpenses = Object.values(results)
    .reduce((sum, cat) => sum + cat.expenses, 0);
  
  for (const category of Object.values(results)) {
    category.percentOfTotal = totalExpenses > 0 
      ? (category.expenses / totalExpenses) * 100 
      : 0;
  }
